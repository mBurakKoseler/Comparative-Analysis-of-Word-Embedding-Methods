...
# PCA nesnesi oluştur
pca = PCA(n_components=101)  # İstediğiniz boyuta göre n_components parametresini ayarlayın

# TF-IDF vektörlerini PCA'ya uygula
...
print("Vektör Boyutu (PCA Sonrası): ", X_pca.shape[1])
...
# TF-IDF vektörünü PCA'ya uygula
user_input_vector_pca = pca.transform(user_input_vector.toarray())

# Kullanıcı girdisi ile veri setindeki sorular arasındaki benzerlikleri hesapla
cosine_similarities = cosine_similarity(user_input_vector_pca, X_pca)

# En benzer soruyu bul
most_similar_index = cosine_similarities.argmax()
most_similar_question = df.iloc[most_similar_index]

print("Kullanıcı Girdisi Vektörü (PCA Sonrası): ", user_input_vector_pca)
print("En Benzer Soru Vektörü (PCA Sonrası): ", X_pca[most_similar_index])
